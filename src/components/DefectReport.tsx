...
export function DefectReport({ data, steelPlates }: DefectReportProps) {
  ...
  const [filterDefectRange, setFilterDefectRange] = useState<{ min?: number; max?: number }>({});
  ...
  const filteredPlates = enrichedPlates.filter(plate => {
    ...
    if (searchQuery) {
      ...
      if (!plate.serialNumber.toLowerCase().includes(query) &&
    ...
    // 级别筛选
    if (filterLevel.length > 0 && !filterLevel.includes(plate.level)) {
      return false;
    }

    // 缺陷数量筛选
    if (filterDefectRange.min !== undefined && plate.defectCount < filterDefectRange.min) {
      return false;
    }
    if (filterDefectRange.max !== undefined && plate.defectCount > filterDefectRange.max) {
      return false;
    }
  ...
  return (
    <div className="bg-card border border-border p-4 h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-medium text-lg">缺陷类型统计 / DEFECT TYPE DISTRIBUTION</h3>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="bg-muted/30 border border-border p-4">
          <div className="text-xs text-muted-foreground mb-1">总检测数 / TOTAL INSPECTIONS</div>
          <div className="text-3xl font-mono">{data.reduce((sum, item) => sum + item.count, 0)}</div>
        </div>
        <div className="bg-muted/30 border border-border p-4">
          <div className="text-xs text-muted-foreground mb-1">缺陷类型 / DEFECT TYPES</div>
          <div className="text-3xl font-mono">{data.length}</div>
        </div>
        <div className="bg-muted/30 border border-border p-4">
          <div className="text-xs text-muted-foreground mb-1">最高频率 / MAX FREQUENCY</div>
          <div className="text-3xl font-mono">{Math.max(...data.map(d => d.count))}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-muted/20 border border-border p-4">
          <h4 className="text-sm font-medium mb-4">缺陷分布图表 / DEFECT DISTRIBUTION CHART</h4>
        ...
        {/* Table */}
        <div className="bg-muted/20 border border-border p-4">
          <h4 className="text-sm font-medium mb-4">缺陷详细列表 / DEFECT DETAIL LIST</h4>
            ...
            {data.map((item, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <span className="text-sm font-mono text-muted-foreground">{item.count} 次</span>
      ...
      <div className="bg-muted/20 border border-border flex-1 flex flex-col min-h-0">
        ...
        <div className="p-3 border-b border-border bg-muted/30 flex items-center justify-between">
          <h4 className="text-sm font-medium">钢板缺陷统计表 / STEEL PLATE DEFECT STATISTICS</h4>
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className={`px-3 py-1.5 text-xs border transition-colors flex items-center gap-1.5 ${
                  filterLevel.length > 0 || filterDefectRange.min !== undefined || filterDefectRange.max !== undefined
              ...
              {showFilterPanel && (
                <div className="absolute right-0 top-full mt-2 w-80 bg-card border-2 border-primary shadow-2xl shadow-primary/20 z-50">
                  <div className="bg-primary/20 border-b-2 border-primary px-3 py-2 flex items-center justify-between">
                    <h4 className="text-xs font-bold uppercase tracking-wider">筛选条件</h4>
                    <button onClick={() => setShowFilterPanel(false)} className="text-xs hover:text-primary">✕</button>
                  </div>
                  <div className="p-3 space-y-3">
                    {/* Level Filter */}
                    <div>
                      <label className="block text-xs text-muted-foreground mb-2">质量级别</label>
                    ...
                    {/* Defect Count Range */}
                    <div>
                      <label className="block text-xs text-muted-foreground mb-2">缺陷数量范围</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                          placeholder="最小"
                          value={filterDefectRange.min || ''}
                          onChange={(e) => setFilterDefectRange(prev => ({ ...prev, min: e.target.value ? parseInt(e.target.value) : undefined }))}
                        ...
                        <input
                          ...
                          onChange={(e) => setFilterDefectRange(prev => ({ ...prev, max: e.target.value ? parseInt(e.target.value) : undefined }))}
                          className="flex-1 px-2 py-1.5 bg-background border border-border text-xs focus:outline-none focus:border-primary"
                        />
                      </div>
                    </div>
            ...
            <span className="text-xs text-muted-foreground">
              {sortedPlates.length}/{enrichedPlates.length} 条记录
            </span>
          </div>
        </div>
          ...
          <table className="w-full text-xs">
            <thead className="bg-muted/50 border-b-2 border-border sticky top-0 z-10">
              <tr>
                ...
                <th className="px-3 py-2 text-left font-bold border-r border-border">钢种</th>
                <th className="px-3 py-2 text-left font-bold border-r border-border">规格(mm)</th>
                <th 
                ...
                {defectTypes.map(type => (
                  <th key={type} className="px-3 py-2 text-center font-bold border-r border-border" title={type}>
                    {type.substring(0, 2)}
                  </th>
                ))}
                <th 
            ...
            <tbody>
              {sortedPlates.length === 0 ? (
                ...
                sortedPlates.map((plate, index) => (
                  <tr 
                    key={plate.plateId}
                    onClick={() => setSelectedPlateId(selectedPlateId === plate.plateId ? null : plate.plateId)}
                  ...
                  >
                    <td className="px-3 py-2 font-mono border-r border-border">{plate.serialNumber}</td>
                    <td className="px-3 py-2 font-mono font-bold border-r border-border">{plate.plateId}</td>
                    <td className="px-3 py-2 font-mono border-r border-border">{plate.steelGrade}</td>
                    ...
                    <td className="px-3 py-2 font-mono text-[10px] border-r border-border">
                      {plate.timestamp.toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}
                    </td>
                    {defectTypes.map(type => (
                      <td 
                        key={type} 
                    ...
                    <td className="px-3 py-2 text-center">
                      <span className={`inline-block px-2 py-0.5 rounded-sm border text-[10px] ${
