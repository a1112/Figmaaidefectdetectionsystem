...
export function FilterDialog({ isOpen, onClose, onFilter, triggerRef }: FilterDialogProps) {
  const [selectedLevels, setSelectedLevels] = useState<('A' | 'B' | 'C' | 'D')[]>([]);
  ...
  const handleApply = () => {
    ...
    if (defectCountMin) filters.defectCountMin = parseInt(defectCountMin);
    if (defectCountMax) filters.defectCountMax = parseInt(defectCountMax);
  ...
  const levelConfig = {
    'A': { bg: 'bg-green-500/10', border: 'border-green-500/30', text: 'text-green-400', activeBg: 'bg-green-500', activeBorder: 'border-green-500' },
    'B': { bg: 'bg-blue-500/10', border: 'border-blue-500/30', text: 'text-blue-400', activeBg: 'bg-blue-500', activeBorder: 'border-blue-500' },
    'C': { bg: 'bg-yellow-500/10', border: 'border-yellow-500/30', text: 'text-yellow-400', activeBg: 'bg-yellow-500', activeBorder: 'border-yellow-500' },
    'D': { bg: 'bg-red-500/10', border: 'border-red-500/30', text: 'text-red-400', activeBg: 'bg-red-500', activeBorder: 'border-red-500' }
  };
      ...
      >
        ...
        <div className={`bg-primary/20 border-b border-primary/50 flex items-center justify-between ${
          ...
          <div className="flex items-center gap-2">
            <Filter className={`text-primary ${isMobileDevice ? 'w-5 h-5' : 'w-4 h-4'}`} />
        ...
        <div className={`space-y-4 ${isMobileDevice ? 'p-4 max-h-[70vh] overflow-auto' : 'p-3 space-y-3'}`}>
              ...
              {(['A', 'B', 'C', 'D'] as const).map((level) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="font-bold text-base font-mono">{level}</div>
                    <div className="text-[8px] mt-0.5 opacity-80">
                      {level === 'A' ? '一等品' : level === 'B' ? '二等品' : level === 'C' ? '三等品' : '等外品'}
                    </div>
                  </button>
                );
              })}
            </div>
            <p className="text-[9px] text-muted-foreground mt-1.5">点击选择一个或多个等级，可多选</p>
          </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setDefectCountMin(e.target.value)}
                    placeholder="0"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setDefectCountMax(e.target.value)}
                    placeholder="∞"
                    className="w-full px-2.5 py-1.5 bg-background border border-border text-xs font-mono focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors"
                  />
                </div>
              </div>
              <p className="text-[9px] text-muted-foreground mt-1.5">设置缺陷数量范围，留空表示不限制</p>
            </div>
          </div>
          ...
          {/* 当前筛选条件显 */}
          {(selectedLevels.length > 0 || defectCountMin || defectCountMax) && (
            <div className="bg-primary/10 border border-primary/30 p-2.5">
              <div className="text-[9px] text-primary font-bold uppercase tracking-wide mb-1">当前筛选条件</div>
              <div className="text-[10px] text-muted-foreground space-y-0.5">
                {selectedLevels.length > 0 && (
                  <div>等级: {selectedLevels.map(level => getLevelText(level)).join('、')}</div>
                )}
                {(defectCountMin || defectCountMax) && (
                  <div>
                    缺陷数: {defectCountMin || '0'} ~ {defectCountMax || '∞'}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-1.5 pt-1">
            ...
            >
              ...
              <span className="font-bold text-[10px] uppercase tracking-wider">应用筛选</span>
            </button>
            <button
              onClick={handleReset}
